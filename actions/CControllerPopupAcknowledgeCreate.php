...
**
**/
namespace Modules\ProblemView\Actions;
...
use CUrl;
use CScreenProblem;
use ServiceNowClient;

class CControllerPopupAcknowledgeCreate extends CAction {
	...
	protected function doAction() {
		...
		$this->create_case = $this->getInput('create_case', 0);
		...
		if($this->create_case == 1) {
			
			$ServiceNow = new ServiceNowClient($this->sn_instance, $this->sn_user, $this->sn_password, '');
			$col = array_column( $events, "severity" );
			array_multisort( $col, SORT_DESC, $events );
			$sn = array();
			
			$description = '';
			foreach($events as $var => $value) {
				$description .= $value['name'] . "\n";
				
			}
			//echo "Case name: " . $events[0]['name'] . "\n";
			...
			$host = API::Host()->get([
				...
				'hostids' => $events[0]['hosts'][0]['hostid'],
				'preservekeys' => true
					]);
			if(!empty($host[$events[0]['hosts'][0]['hostid']]['inventory']['serialno_a'])) {
				$serial = $host[$events[0]['hosts'][0]['hostid']]['inventory']['serialno_a'];
			}
			else {
				$serial = $host[$events[0]['hosts'][0]['hostid']]['inventory']['serialno_b'];
			}
			
			
			switch ($events[0]['severity']) {
				case 5:
					$events[0]['severity'] = 1;
					break;
				case 4:
			...
			$data_array = array(
				'short_description' => $events[0]['name'],
				'description' => $description,
				'urgency' => $events[0]['severity'],
				'impact' => $events[0]['severity'],
				'severity' => $events[0]['severity'],
				'state' => 'new',
				'u_hostname' => $events[0]['hosts'][0]['name'],
				'device' => $events[0]['hosts'][0]['name'],
				'agent'	=> 'Zabbix',
				'u_serial_number'	=> $serial,
				'assigned_to' => 'SMC Networking',
				'firsttime'		=> date( "H:i:s d/m/Y", $events[0]['clock']),
				'message'	=> $this->message,
				'ipaddress'	=> $host[$events[0]['hosts'][0]['hostid']]['interfaces'][0]['ip']
			  );

			  try {
				  $ServiceNow->CreateRecord("incident", $data_array);
			  } catch (Throwable $t)
			  {
	...
	private function sortEventsOnSeverity($array, $key) {
		
		$col = array_column( $array, 'eventid');
		return array_multisort( $col, SORT_ASC, $array );
	...
	protected function getEventDetails(array $eventids) {
		...
		$events = API::Event()->get([
			'output' => ['name', 'value', 'clock', 'severity', 'eventid', 'objectid', 'acknowledged', 'r_eventid'],
